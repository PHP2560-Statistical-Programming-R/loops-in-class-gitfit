...
```{r}
# create a function of the name bubble_sort by having vec as a variable
bubble_sort = function(vec) {
    no_passes = 0
  #set no passes=0
    while(1) {
    #while loop is used to continue running the function if no swaps is greater than 0
        no_swaps = 0
        #initialize the vector and set no_swaps=0
        for (j in 1 : (length(vec) - 1 - no_passes)) {
          #initialize the for loop where it loops from the start through the entire length of the vector
            if (vec[j] > vec[j + 1]) {
              #if the vector satisifies this statement where j>j+1 then it will output the function listed below
                s = vec[j]
                #s equals the number within the j position
                vec[j] = vec[j+1]
                #updating the value in the j position to j+1
                vec[j+1] = s
                #the new value of s is equal to j+1
                no_swaps = no_swaps + 1
                #updates no_swaps by adding 1 to it
            }
        }
        no_passes = no_passes + 1
        #updates no_passes by adding 1 to it
        if(no_swaps == 0) break
        #if no_swaps=0 then it will "break" or stop looping and print out vec
    }
    vec
}
```
...
-------------------------------------------------------
```{r}
table_1<-matrix('',nrow=100000)
for(i in 1:nrow(table_1)){
 table_1[i]=result(sample(1:6, size=5, replace=T))
}
table(table_1)/nrow(table_1)
```
