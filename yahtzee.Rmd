...
- Four of a Kind (four of one number and one other) [x]
- Yahtzee (five of the same number) [x]
- Small Straight (four consecutive numbers) [ ]
- Large Straight (five consecutive numbers) [x]
- Chance (anything that does not fit the above patterns) [x]
...
```{r}
bubble_sort = function(vec) { # create function with an argument vec
    no_passes = 0 # set no_passes to 0
    while(1) { # iterate
        no_swaps = 0 # set no_swaps to 0
        for (j in 1 : (length(vec) - 1 - no_passes)) { # for the next element
            if (vec[j] > vec[j + 1]) { # if the element is greater than the next element
                s = vec[j] # store element in temporary variable
                vec[j] = vec[j+1] # put next element in current element's spot
                vec[j+1] = s # put temporary variable element in next element's spot (ie swap the two elements)
                no_swaps = no_swaps + 1 # increment the number of swaps
            }
        }
        no_passes = no_passes + 1 # increment the number of passes
        if(no_swaps == 0) break # stop iterating when the number of swaps is 0
    }
    vec
}
```
...
} else if (x[3] == x[2] + 1 & x[4] == x[3] + 1 & (x[2] == x[1] + 1 | x[5] == x[4] + 1) | 
...
hand <- character(100000)
for (i in 1:100000)
  {
  x[i,] <- sample(1:6, size=5, replace=T)
  x[i,] <- bubblesort(x[i,])
  hand[i] <- hand_type(x[i,])
}
