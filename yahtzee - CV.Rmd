...
```{r}
bubble_sort = function(vec) { # The fuction takes a vector as an input 
    no_passes = 0 # initialize no_passes
    while(1) { # run the code below while TRUE (infinite)
        no_swaps = 0 # initialize no_swaps
        for (j in 1 : (length(vec) - 1 - no_passes)) { # the loop runs over values of j from 1 to length(vec)-1-no_passes
            if (vec[j] > vec[j + 1]) { # if the condition vec[j] > vec[j + 1] is met, then do the following:
                s = vec[j] # s is equal to the number that is in the jth position in vec, which is a temporary holding variable to swap the order 
                vec[j] = vec[j+1] # update the number that is in the jth position in vec with the number that is in the jth+1 position in vec
                vec[j+1] = s # vec[j+1] equals s, 
                no_swaps = no_swaps + 1 # update the vector no_swaps by increasing the value by 1, so that the for loop keep running
            }
        }
        no_passes = no_passes + 1 # update the vector no_passes by increasing the value by 1, so that the if loop keep running
        if(no_swaps == 0) break # stop the while loop when no_swaps==0
    }
    vec # print the final vector
}
  ...
  hand="Yahtzee"
    ...
    hand="4ofKind"
      ...
      hand="3ofKind"
        ...
        hand="FullHouse"
        } else if ((x[1]+1==x[2] & x[2]+1==x[3] & x[3]+1==x[4])  | (x[2]+1==x[3] & x[3]+1==x[4] & x[4]+1 == x[5])) {
          hand="Small Straight"
...
names(wcgs2)
WCGS.new <- wcgs2 %>%
  select(-id, -behpat, -dibpat, -chd69, -typchd69, -arcus)
...
```{r}
for (i in 1:nrow(WCGS.new)) {
  WCGS.new[i,] <- standardize.me(x[i,])
}
```
...
for (i in 1:ncol(WCGS.new)){
  avg <- mean(WCGS.new[,i], na.rm=TRUE)
  std.dev <- sd(WCGS.new[,i], na.rm=TRUE)
}
...
for (i in 1:nrow(wcgs2)){
  if (CHD==1){
    avg2 <- mean(wcgs2$weight)
  } else{
    avg3 <- mean(wcgs2$weight)
  }
}
...
# Alternative 2
aggregate(weight~chd69, data=wcgs2, FUN=function(x) c(mean(x)))
