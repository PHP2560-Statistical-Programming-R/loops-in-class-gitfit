...
```{r}
#create a function of variable vec and name it bubble_sort
bubble_sort = function(vec) {
#set no_passes equal to 0
    no_passes = 0
#while the condition is true
    while(1) {
#set no_swaps equal to 0
        no_swaps = 0
#initialize for the for loop where it loops from the start through the entire length of the vector ((length(vec)-1-no_passes)
        for (j in 1 : (length(vec) - 1 - no_passes)) {
#if the number in the (j+1)th position in vec is greater than the number in the jth position
            if (vec[j] > vec[j + 1]){
#then s is equal to the jth value of s
                s = vec[j]
#update the jth number by the value of (j+1)th number
                vec[j] = vec[j+1]
#then rename vec[j+1] as s
                vec[j+1] = s
#update no_swaps by adding 1 to it
                no_swaps = no_swaps + 1
            }
        }
#update no_passes by adding 1
        no_passes = no_passes + 1
#if no_swaps is equal to 0,then the loop stops
        if(no_swaps == 0) break
    }
    vec
}
...
if (TRUE && x[1] == x[5]){
  ...
  hand = "3ofkind"
}else if (( (x[4]==x[3]+1 & x[3] == x[2]+1 & x[2]==x[1]+1) & x[5]!=x[1]+4) | (x[5]==x[4]+1 & x[4]==x[3]+1 & x[3] == x[2] +1 & x[5] != x[1]+4 )| (x[1]==x[2] & x[3]==x[2]+1 & x[4]==x[3]+1 & x[5]==x[4]+1) | (x[2]==x[3] & x[4]==x[3]+1 & x[5]==x[4]+1 & x[2]==x[1]+1) | (x[3]==x[4] & x[3]==x[2]+1 & x[2]==x[1]+1 & x[5]==x[4]+1) | (x[5]==x[4] & x[4]==x[3]+1 & x[3]==x[2]+1 & x[2]==x[1]+1) ){
  hand = "SmallStraight"
}else if (x[5]== x[1]+4 & x[5] == x[4]+1 & x[4]==x[3]+1 & x[3]==x[2]+1 & x[2]==x[1]+1){
  hand = "LargeStraight"
}else{
  hand = "chance"
}
...
-------------------------------------------------------
```{r}
m <- matrix(data='', nrow=100000)
 for (i in 1:nrow(m)) {
  m[i] <- y(sample(1:6, size=5, replace=TRUE))
 }

Yahtzee = sum(m=="Yahtzee")/100000
Fourofakind = sum(m=="4ofKind")/100000
Threeofakind=sum(m=="3ofkind")/100000
Fullhouse = sum(m=="FullHouse")/100000
largestraight = sum(m=="LargeStraight")/100000
SmallStraight = sum(m=="SmallStraight")/100000
chance<-sum(m=="chance")/100000
