...
bubble_sort = function(vec) {
    ...
    while(1) { #While TRUE
        no_swaps = 0
        for (j in 1 : (length(vec) - 1 - no_passes)) { #start vec-1-0 ineration for the first element (, start vec-1-1 and so on)
            if (vec[j] > vec[j + 1]) { #if the number in the previous position is larger than the position in the next position, then do this: 
                s = vec[j] #create temporary variable
                vec[j] = vec[j+1] #swap element place by putting the larger element in the next slot
                vec[j+1] = s #place the smaller element in the current slot
                no_swaps = no_swaps + 1 #add swap counter 
            }
        }
        no_passes = no_passes + 1 #for every ineration, count passed element
        if(no_swaps == 0) break #if no_swap is zero, then no need to swap, end while loop
    }
    vec #return vector
}
...
hand_name<-function(x){
if (x[1]==x[5]){
  hand="Yahtzee"
} else if (x[1]==x[4] | x[2]==x[5]) {
    hand="4ofKind"
} else if (x[1]==x[3] | x[2]==x[5] | x[3]==x[5]){ 
  hand="3ofKind"
    ...
    hand="Large Straight"
           ...
           hand="Small Straight"
}else if (x[1]==x[2] & x[3]==x[5] | x[4]==x[5] & x[1]==x[3]){
   hand="Full House"
} else {
 hand="Chance"
}
...
```{r}
x <- matrix('', nrow=100000, ncol=1)
for (i in 1:nrow(x)){
  y<-bubblesort(sample(1:6, size=5, replace=T))
  x[i] <- hand_name(y)
}
table(x)/100000
```
