...
#bubble_sort = function(vec) { # Creating a function called bubble sort that takes the variable vec as an input
 #   no_passes = 0 # defining initial values. The number of passes
  #  while(1) { # this tells R to run the function if the condiiton  no_swaps >0 is met
   #     no_swaps = 0 #initialize the vector no_swaps which counts the number of swaps
    #    for (j in 1 : (length(vec) - 1 - no_passes)) { # the loop runs the length of(vec) -1 -nopasses
     #       if (vec[j] > vec[j + 1]) { # This sets up the inner if statement. SO if vec[j]>vec[j+1] then
      #          s = vec[j] # s is equal to the number in the jth position of vec
       #         vec[j] = vec[j+1] # and vec now equals the number in the jth+1 position
        #        vec[j+1] = s # this renames the variable s, which is a temporary holding variable 
         #       no_swaps = no_swaps + 1 # this updates the vector no_swaps by incrementing by 1 so the loop can keep running
          #  }
        #}
        #no_passes = no_passes + 1 # the vector no_passes is incremented by 1 so the loop can continue
        #if(no_swaps == 0) break # Break from the for loop if the variable no_swaps is equal to 0
   # }
    #vec # print the final result "vec"
#}
```
...
}else if (x[4]==x[5]& x[1]==x[3] | x[3]==x[5]& x[1]==x[2] ){
  hand="FullHouse"
}else if (x[1]==x[3]| x[2]==x[4]|x[3]==x[5]){
  hand="3ofKind"
  }else if (x[2] == x[1] + 1 & x[3] == x[2] + 1 & x[4] == x[3] + 1 & x[5] == x[4] + 1) {
  hand = "LargeStraight"
} else if (x[2]==(x[1]+1) & x[3]==(x[2]+1) & x[4]==(x[3]+1) |
