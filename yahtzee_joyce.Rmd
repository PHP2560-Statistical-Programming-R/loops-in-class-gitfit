...
# if this statement holds then give x as the output
for(last in length(x):2) {
for(first in 1:(last - 1)) {
if(x[first] > x[first + 1]) { # This says if a number is larger then the one that comes before it, then swap the two numbersand save it in a temporary holding place 
save <- x[first] # this vector is the temporary holding for the swaps we make
x[first] <- x[first + 1] #the first number is replaced by the number that comes after it
x[first + 1] <- save #the second number is then replaced by the one that inititally came before it
}
}
}
return (x)
...
-------------------------------------------------------
```{r}
answer<-matrix(,nrow=100000)
for (i in 1:nrow(answer)){
  answer[i,]=namer(sample(1:6,size=5,replace=T))
}
table(answer)/nrow(answer)
